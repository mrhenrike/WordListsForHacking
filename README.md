...
├── passwords/
│   ├── wlist_brasil.lst           # Brazilian passwords (~1.70M unique)
...
|------|-------------|---------|
| `passwords/wlist_brasil.lst` | Brazilian password corpus — fully generated by the WFH tool using cultural word banks, behavioral patterns, leet speak permutations, keyboard walks, and Portuguese dictionary. All entries are program-generated patterns, sanitized and deduplicated. Min 5 chars. | ~1.70M |
...
The file `passwords/wlist_brasil.lst` is a curated Brazilian password corpus with **~1.70 million unique entries**, fully generated by the WFH tool using its pattern, profile, charset, leet speak, and cultural word bank modules.
...
| **Pattern Generation** | Template-based patterns: `{name}{year}`, `{word}{separator}{number}`, etc. |
...
- Generic corporate credential patterns (role+year, department+number)

### Sanitization Rules Applied

- Minimum 5 characters
- No specific company names, brands, or identifiable corporate data
- No real PII, CNPJs, CPFs, or email addresses
- No entries from external leak/breach databases
- Fully deduplicated
